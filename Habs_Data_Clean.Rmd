...
```{r lms SoCal}
# create lms for pn and alex for whole SoCal region
lm_pn <- step(lm(data = df_habs_interp %>% drop_na(), pn ~ . -alex-date))

SC_pn_lm <- as.data.frame(tidy(lm_pn)) %>% 
    select(term, estimate) %>% 
    pivot_wider(names_from = term, values_from = estimate)

lm_alex <- step(lm(data = df_habs_interp %>% drop_na(), alex ~ . -pn-date))

SC_al_lm <- as.data.frame(tidy(lm_alex)) %>% 
    select(term, estimate) %>% 
    pivot_wider(names_from = term, values_from = estimate)

#df of lm coefficients
SC_lm <- bind_rows(SC_pn_lm, SC_al_lm)
```
```{r glms cities}

#create lms and plots of pn and alex [] for each city
...
cities <- c("CalPoly", "StearnsWharf", "SantaMonicaPier", "NewportPier", "ScrippsPier")

#for loop to generate plots and lms
for (i in seq(length(cities))) {
  
  df <- df_habs_interp %>% filter(location == cities[i]) %>% drop_na()
  dfs[[i]] <- df
  
  pns[[i]] <- step(lm(data = df, pn ~ nh4 + no3 + no2 + po4 + sio3 + temp + month + year)) 
  pns_plots[[i]] <- ggplot(df, aes(x = date, y = pn)) + geom_line() + geom_smooth(method = 'lm', se= F) + labs(title = "Pseudo. over Time", subtitle = cities[i], x = "Date", y = "Concentration (uM)")
  
  alexs[[i]] <- step(lm(data = df, alex ~ nh4 + no3 + no2 + po4 + sio3 + temp + month + year))
  alexs_plots[[i]] <- ggplot(df, aes(x = date, y = alex)) + geom_line() + geom_smooth(method = 'lm', se = F) + labs(title = "Alex. over Time", subtitle = cities[i], x = "Date", y = "Concentration (uM)")
  
}
#plots of pns and alexs for each city
plot_grid(plotlist = pns_plots)
plot_grid(plotlist = alexs_plots)
...
#dfs of pn lms coeffs and alex lm coeffs
pn_lms <- cbind(cities, do.call(bind_rows, lm_pn_cities)) %>% select(-"(Intercept)")
al_lms <- cbind(cities, do.call(bind_rows, lm_al_cities)) %>% select(-"(Intercept)")

...
df_habs <- st_drop_geometry(hab_occ_wq) %>% mutate(month = month(eventDate), year = year(eventDate))

species_labs <- paste0(c("alex", "pn_ser"), "_c_L")

u_m <- paste0(c("NH4", "NO3", "NO2", "PO4", "SiO3"), "_u_M")
colnames(df_habs) <-  append(append(append(c("location", "sample_id"), species_labs), append(u_m, c("T_C", "date"))), c("month", "year"))


summary(df_habs)

df_habs_interp <- df_habs %>% group_by(location) %>% 
  mutate(alex = na.approx(alex_c_L, na.rm  = F),
         pn = na.approx(pn_ser_c_L, na.rm  = F),
         NH4 = na.approx(NH4_u_M, na.rm  = F), 
         NO3 = na.approx(NO3_u_M , na.rm  = F),
         NO2 = na.approx(NO2_u_M, na.rm  = F),
         PO4 = na.approx(PO4_u_M, na.rm  = F),
         SiO3 = na.approx(SiO3_u_M, na.rm  = F)) %>% drop_na()

sst <- ggplot(df_habs, aes(x = date, color = location)) + geom_line(aes(y = T_C)) + labs(title = "SST over Time", y ="Temperature (C)", x = "Date", color = "Location")
pn <- ggplot(df_habs, aes(x = date, color = location)) + geom_line(aes(y = pn_ser_c_L)) + labs(title = "P. seriata over Time", y ="Concentration (uM)", x = "Date", color = "Location")
alex <- ggplot(df_habs, aes(x = date, color = location)) + geom_line(aes(y = alex_c_L)) + labs(title = "Alexandrium over Time", y ="Concentration (uM)", x = "Date", color = "Location")
...
lm_pn <- lm(data = df_habs, pn_ser_c_L ~ . - location - sample_id)

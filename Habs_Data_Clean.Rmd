...
hab_occ_wq <- merge(hab_occ, water_qual, by = c('id')) %>% 
  ...
  select("id", "locationID", "eventDate", "Temp ( Celsius )" , "Akashiwo sanguinea ( cells/L )", "Alexandrium ( cells/L )", "Dinophysis ( cells/L )", "Lingulodinium polyedra ( cells/L )", "Pseudo-nitzschia delicatissima ( cells/L )", "Pseudo-nitzschia seriata ( cells/L )", "longitude", "latitude") %>% 
  rename_with(~gsub("\\s", "", .x)) %>% 
  st_as_sf(coords = c('longitude', 'latitude'), crs = 4326) %>% 
  filter(eventDate >= "2011-10-05" & eventDate <= "2020-03-02", locationID != "SantaCruzWharf") %>% drop_na()
...
species_labs <- paste(c("A. Sang.", "Alex.", "Dino.", "Ling.", "Pn. Del.", "Pn. Ser."), "(Cells/L)")

df_habs <- st_drop_geometry(hab_occ_wq)

colnames(df_habs) <-  append(c("Sample ID", "Location ID", "Event Date", "Temperature (Celsius)"), species_labs)
summary(df_habs)
...
df_habs_pois <- df_habs %>% 
  mutate(A_sang_HAB = ifelse(df_habs$`A. Sang. (Cells/L)` > quantile(df_habs$`A. Sang. (Cells/L)`, 0.75), 1, 0),
         Alex_HAB = ifelse(df_habs$`Alex. (Cells/L)` > quantile(df_habs$`Alex. (Cells/L)`, 0.75), 1, 0),
         Dino_HAB = ifelse(df_habs$`Dino. (Cells/L)` > quantile(df_habs$`Dino. (Cells/L)`, 0.75), 1, 0),
         Ling_HAB = ifelse(df_habs$`Ling. (Cells/L)` > quantile(df_habs$`Ling. (Cells/L)`, 0.75), 1, 0),
         Pn_Del_HAB = ifelse(df_habs$`Pn. Del. (Cells/L)` > quantile(df_habs$`Pn. Del. (Cells/L)`, 0.75), 1, 0),
         Pn_Ser_HAB = ifelse(df_habs$`Pn. Ser. (Cells/L)` > quantile(df_habs$`Pn. Ser. (Cells/L)`, 0.75), 1, 0)
         )
...
SST <- ggplot(hab_occ_wq, aes(x = eventDate, y = `Temp(Celsius)`, color = locationID, group = locationID)) + geom_point(size = 0.2) + geom_smooth(method = 'lm', se = F, size = 0.7)

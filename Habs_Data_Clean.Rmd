...
lm_pn <- step(lm(data = df_habs_interp, pn ~ . -alex-date))
summary(lm_pn)
...
for (i in seq(length(cities))) {
  ...
  lm_pn <- step(lm(data = df, pn ~ nh4 + no3 + no2 + po4 + sio3 + temp + month + year))
  summary(lm_pn)
  lm_alex <-  step(lm(data = df, alex ~ nh4 + no3 + no2 + po4 + sio3 + temp + month + year))
  dfs[[i]] <- df
  pns[[i]] <- lm_pn
  alexs[[i]] <- lm_alex

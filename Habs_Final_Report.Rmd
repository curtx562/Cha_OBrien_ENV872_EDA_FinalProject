---
<<<<<<< HEAD
title: "Harmful Algal Blooms in Southern California: a Time Series Analysis"
=======
title: "Harmful Algal Blooms in Souther California"
subtitle: "https://github.com/curtx562/Cha_OBrien_ENV872_EDA_FinalProject"
>>>>>>> 4366b8c66a2c0ef737e1895dcc34b1cbb1ddeb0f
...
kable(head(df_hab_table), caption = "Table 1: First rows of processed data", booktabs=TRUE, format="latex", longtable=TRUE) %>%
  kable_styling(latex_options="HOLD_position")

kable(na, caption = "Table 2: Number of NA's in processed dataset", booktabs=TRUE, format="latex", longtable=TRUE) %>%
  kable_styling(latex_options="scale_down")

kable(date, caption = "Table 3: Date range of avaiable data for each sample location", booktabs=TRUE, format="latex", longtable=TRUE) %>%
  kable_styling(latex_options="scale_down")
```
...
Ten time series analyses were performed in this project: five examining changes in Pseudo-nitzschia concentrations over time at each SCCOOS site and five examining changes in Alexandrium concentrations over time at each SCCOOS site. These analyses were conducted by first filtering HAB data for a specific SCCOOS site and then generating a daily dataset encompassing that site’s date range. The filtered dataset and daily dataset were then merged using “eventDate” as the shared column, and a linear interpolation was used to fill missing HAB data. We felt as though a linear interpolation was appropriate in this case because missing data gaps were not large but differences in algal concentrations were significant. Next, a monthly mean dataset was generated using the interpolated HAB values; this was the dataset used to establish starting points, create the time series object and run the seasonal Mann Kendall test. This process was used to create and test each time series object.
...
```{r Alexandrium in Newport Pier: time series, echo = FALSE, eval = FALSE}
#specify location
...
```{r Alexandrium in San Luis Obispo: time series, echo = FALSE, eval = FALSE}
#specify location
CalPoly <- hab_occ_wq %>%
...
```{r Alexandrium in Santa Monica: time series, echo = FALSE, eval = FALSE}
#specify location
...
```{r Alexandrium in San Diego: time series, echo = FALSE, eval = FALSE}
#specify location
SanDiego <- hab_occ_wq %>%
...
```{r Alexandrium in Santa Barbara: time series, echo = FALSE, eval = FALSE}
#specify location
...
```{r Pseudo-nitzschia seriata in San Luis Obispo: time series, echo = FALSE, eval = FALSE}
#specify location
CalPoly <- hab_occ_wq %>%
  filter(locationID == "CalPoly")
...
```{r Pseudo-nitzschia seriata in Santa Monica: time series, echo = FALSE, eval = FALSE}
#specify location
SantaMonica <- hab_occ_wq %>%
  filter(locationID == "SantaMonicaPier")
...
```{r Pseudo-nitzschia seriata in San Diego: time series, echo = FALSE}
#specify location
SanDiego <- hab_occ_wq %>%
  filter(locationID == "ScrippsPier")
...
```{r Pseudo-nitzschia seriata in Newport: time series, echo = FALSE, eval = FALSE}
#specify location
NewportPier <- hab_occ_wq %>%
  filter(locationID == "NewportPier")
...
```{r Pseudo-nitzschia seriata in Santa Barbara: time series, echo = FALSE, eval = FALSE}
#specify location
SantaBarbara <- hab_occ_wq %>%
  filter(locationID == "StearnsWharf")
...
pn_lms <- cbind(cities, do.call(bind_rows, lm_pn_cities)) %>% select(-"(Intercept)") %>% cbind(lm_pn_r) %>% select(cities, no3, no2, sio3, po4, temp, month, year, lm_pn_r)

colnames(pn_lms) <- c("Site", "NO3", "NO2", "SiO3", "PO4" ,"Temp.", "Month", "Year", "R-sq.")

#colnames(pn_lms) <- c("")

write.csv(pn_lms, "./Data/Output/PN_LM_Coefficients_Cities.csv")

al_lms <- cbind(cities, do.call(bind_rows, lm_al_cities)) %>% select(-"(Intercept)") %>% cbind(lm_al_r) %>% select(cities, no3, sio3, po4, month, year, lm_al_r)

colnames(al_lms) <- c("Site", "NO3", "SiO3", "PO4", "Month", "Year", "R-sq.")

write.csv(al_lms, "./Data/Output/AL_LM_Coefficients_Cities.csv")

```

```{r LM Output Tables, echo = FALSE}
kable(pn_lms, caption = "Table 3: Coefficients for Pseudo-nitzschia Linear Models", booktabs=TRUE, format="latex", longtable=TRUE) %>%
  kable_styling(latex_options="HOLD_position")

kable(pn_lms, caption = "Table 4: Coefficients for Alexandrium Linear Models", booktabs=TRUE, format="latex", longtable=TRUE) %>%
  kable_styling(latex_options="HOLD_position")
```
...
Moore et al. 2008: Impacts of climate variability and future climate change on harmful algal blooms and human health 

Lewitus et al. 2012: Harmful algal blooms along the North American west coast region: History, trends, causes, and impacts

Zhu et al. 2017: Understanding the blob bloom: Warming increases toxicity and abundance of the harmful bloom diatom Pseudo-nitzschia in California coastal waters

Smith et al. 2017: A decade and a half of Pseudo-nitzschia spp. and domoic acid along the coast of southern California

Trainer et al. 2020: Pelagic harmful algal blooms and climate change: Lessons from nature’s

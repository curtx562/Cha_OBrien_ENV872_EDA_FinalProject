...
df_habs <- hab_occ_wq %>% 
  ...
  select(eventDate, locationID, "Alexandrium ( cells/L )", "Pseudo-nitzschia seriata ( cells/L )", "Ammonium ( uM )", "Nitrate ( uM )", "Nitrite ( uM )", "Phosphate ( uM )" , "Silicate ( uM )" , "Temp ( Celsius )", "month", "year") %>% drop_na()
...
knitr::kable(head(df_hab_table), caption = "Table of Processed Data")
```
...
plot_grid(explore1, explore2, ncol = 1)

legend <- get_legend(scatter_alex_t)

plot_grid(explore_3, legend, rel_widths = c(1, .1))

```

```{r Exploratory Analysis 2, echo = FALSE}

nh4 <- ggplot(df_habs_interp, aes(x = date, color = location)) + geom_line(aes(y = nh4)) + labs(subtitle = "NH4", y ="uM", x = "Date", color = "Location") +  theme(axis.title.x = element_blank(), legend.spacing.y = unit(0, "mm"),
        legend.background = element_blank(),
        legend.box.background = element_rect(colour = "black"))

no3 <- ggplot(df_habs_interp, aes(x = date, color = location)) + geom_line(aes(y = no3)) + labs(subtitle = "NO3", y ="Temperature (C)", x = "Date", color = "Location") +  theme(axis.title.x = element_blank(), axis.title.y = element_blank(), legend.position="none")

no2 <- ggplot(df_habs_interp, aes(x = date, color = location)) + geom_line(aes(y = no2)) + labs(subtitle = "NO2", y ="Temperature (C)", x = "Date", color = "Location") +  theme(axis.title.x = element_blank(), axis.title.y = element_blank(), legend.position="none")

po4 <- ggplot(df_habs_interp, aes(x = date, color = location)) + geom_line(aes(y = po4)) + labs(subtitle = "PO4", y ="uM", x = "Date", color = "Location") +  theme( legend.position="none")

sio3 <- ggplot(df_habs_interp, aes(x = date, color = location)) + geom_line(aes(y = sio3)) + labs(subtitle = "SiO3", y ="Temperature (C)", x = "Date", color = "Location") +  theme(axis.title.y = element_blank(),legend.position="none") 

sst <- ggplot(df_habs_interp, aes(x = date, color = location)) + geom_line(aes(y = temp)) + labs(title = "SST Over Time", y ="Temperature (C)", x = "Date", color = "Location") + theme(legend.spacing.y = unit(0, "mm"),
        legend.background = element_blank(),
        legend.box.background = element_rect(colour = "black"))
plot_grid(nh4 + theme(legend.position = "none"), no3, no2, po4, sio3, get_legend(nh4))
...
write.csv(pn_lms, "./Data/Output/PN_LM_Coefficients_Cities2.csv")

al_lms <- cbind(cities, do.call(bind_rows, lm_al_cities)) %>% select(-"(Intercept)")

write.csv(al_lms, "./Data/Output/AL_LM_Coefficients_Cities2.csv")
```
